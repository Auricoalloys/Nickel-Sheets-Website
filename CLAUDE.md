...
before it holds anything, and then a deliberate choice: world-readable like `busbarproduct`, or
no-access like `leads`. The advisory email lists one table per card and is easy to scroll past — the
second query at the end of that migration lists every public table still missing RLS in one go.
